...
/*Função que exibe os livros do array books na tabela/
...
/*Função que exibe os livros que forem sendo adicionados, por meio do formulário, na tabela/
function exibirDadosTabela(event){
    ...
    for(let book = books.length-1; book <= books.length-1; book++){
        if(books[book].isbn === "" || books[book].title === "" || books[book].author ==="" || books[book].published ==="") {
            ...
            /*Se os dados desse último elemento não forem vazios, ele continua sendo o último elemento do array books*/
